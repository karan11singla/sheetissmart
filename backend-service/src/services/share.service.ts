...
interface ShareSheetInput {
  ...
  permission: 'VIEWER' | 'EDIT' | 'EDIT_CAN_SHARE';
  sharedById: string;
}
...
export async function shareSheet(input: ShareSheetInput) {
  ...
  // Check if user has EDIT_CAN_SHARE access
  ...
  if (!isOwner && !shareAccess) {
    throw new AppError('You need EDIT_CAN_SHARE permission to share this sheet', 403);
  }
...
export async function getSheetShares(sheetId: string, userId: string) {
  ...
  // Check if user has any access to this sheet (owner or shared with them)
  const isOwner = sheet.userId === userId;
  const hasAccess = await prisma.sheetShare.findFirst({
    where: {
      sheetId,
      sharedWithId: userId,
    },
  ...
  if (!isOwner && !hasAccess) {
    throw new AppError('Access denied. You must have access to this sheet to view shares.', 403);
  }
