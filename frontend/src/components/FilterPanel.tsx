      ...
      <div className="px-4 py-3">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm font-medium text-neutral-700">Filter by column</span>
          </div>
          <div className="flex items-center space-x-2">
            <button
              onClick={onClearAll}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
          {columns.map((column) => (
            <div key={column.id} className="relative">
              <label className="block text-xs font-medium text-neutral-500 mb-1 truncate">
                {column.name}
              </label>
              <div className="relative">
                <input
                  ...
                  placeholder="Filter..."
                  className="w-full text-sm border border-neutral-200 rounded px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-primary-500 focus:border-primary-500 pr-6"
                />
                {localFilters[column.id] && (
                  <button
                    onClick={() => handleClearColumn(column.id)}
                    className="absolute right-1 top-1/2 -translate-y-1/2 text-neutral-400 hover:text-neutral-600 p-0.5"
                  >
                    <X className="h-3 w-3" />
                  </button>
                )}
